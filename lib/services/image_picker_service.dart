...
class ImagePickerService {
  ...
  static Future<Map<String, dynamic>?> pickImageFromGallery() async {
    debugPrint(
        '📸 ImagePickerService.pickImageFromGallery() - kIsWeb: $kIsWeb');
    ...
    try {
      if (kIsWeb) {
        ...
        debugPrint(
            '📸 FilePicker result: ${result?.files.length ?? 0} arquivos');

        if (result != null && result.files.isNotEmpty) {
          final file = result.files.first;
          debugPrint(
              '📸 Arquivo selecionado: ${file.name}, bytes: ${file.bytes?.length}');
