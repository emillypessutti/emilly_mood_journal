...
----------------------------------------
Os quatro parâmetros abaixo devem ser preenchidos antes de rodar o gerador. A seguir incluímos valores concretos já calculados para cada entidade presente no projeto.
...
Preenchimentos sugeridos para entidades existentes
-------------------------------------------------
1. UserProfileEntity
  - SUFFIX=UserProfiles
  - ENTITY=UserProfileEntity
  - DEST_DIR=lib/features/user_profiles/domain/repositories/
  - IMPORT_PATH=../../../domain/entities/user_profile_entity.dart

2. MoodEntryEntity
  - SUFFIX=MoodEntries
  - ENTITY=MoodEntryEntity
  - DEST_DIR=lib/features/mood_entries/domain/repositories/
  - IMPORT_PATH=../../../domain/entities/mood_entry_entity.dart

3. DailyGoalEntity
  - SUFFIX=DailyGoals
  - ENTITY=DailyGoalEntity
  - DEST_DIR=lib/features/daily_goals/domain/repositories/
  - IMPORT_PATH=../../../domain/entities/daily_goal_entity.dart

4. MoodStatisticsEntity
  - SUFFIX=MoodStatistics
  - ENTITY=MoodStatisticsEntity
  - DEST_DIR=lib/features/mood_statistics/domain/repositories/
  - IMPORT_PATH=../../../domain/entities/mood_statistics_entity.dart

Exemplo rápido (gerar repositório para MoodEntry):
```
SUFFIX=MoodEntries
ENTITY=MoodEntryEntity
DEST_DIR=lib/features/mood_entries/domain/repositories/
...
Depois de definido, o arquivo alvo ficará em:
`lib/features/mood_entries/domain/repositories/mood_entry_repository.dart`

Observação: como as entidades hoje estão centralizadas em `lib/domain/entities/`, os imports relativos sob `lib/features/<plural>/domain/repositories/` exigem três "../" para voltar à pasta `lib/` e acessar `domain/entities`. Caso a estrutura mude futuramente para colocar entidades dentro de cada feature, ajuste somente o IMPORT_PATH.
